<script lang="ts">
	import { onMount } from "svelte";
	import { toast } from "svelte-sonner";
	import { getBlockedUsers, unblockUser } from "$lib/api/account";
	import UserAvatar from "$lib/components/UserAvatar.svelte";
	import * as Item from "$lib/components/ui/item";
	import { Button } from "$lib/components/ui/button";
	import { SpinnerGap, UserCircleIcon } from "phosphor-svelte";

	let users = $state<{ profileId: number; displayName?: string | null; profileImageMediaHash?: string | null }[]>([]);
	let loading = $state(true);
	let unblocking = $state<Record<number, boolean>>({});

	onMount(async () => {
		try {
			const res = await getBlockedUsers();
			users = res.profiles ?? [];
		} catch (e) {
			console.error(e);
			toast.error("Failed to load blocked users");
		} finally {
			loading = false;
		}
	});

	async function onUnblock(profileId: number) {
		unblocking[profileId] = true;
		try {
			await unblockUser(profileId);
			users = users.filter((u) => u.profileId !== profileId);
			toast.success("User unblocked");
		} catch (e) {
			console.error(e);
			toast.error("Failed to unblock");
		} finally {
			unblocking[profileId] = false;
		}
	}
</script>

<div class="flex w-full px-4">
	<main class="pb-(--content-pb) flex flex-col gap-2 w-full max-w-120 m-auto pt-2">
		{#if loading}
			<div class="flex justify-center py-16">
				<SpinnerGap class="size-6 animate-spin text-muted-foreground" />
			</div>
		{:else if users.length === 0}
			<div class="flex flex-col items-center gap-3 py-16 text-center">
				<UserCircleIcon class="size-10 text-muted-foreground/40" />
				<p class="text-sm text-muted-foreground">No blocked users</p>
			</div>
		{:else}
			{#each users as user (user.profileId)}
				<Item.Root variant="outline">
					<Item.Media>
						<UserAvatar mediaHash={user.profileImageMediaHash ?? null} size="md" />
					</Item.Media>
					<Item.Content>
						<Item.Title class="truncate">{user.displayName ?? `User #${user.profileId}`}</Item.Title>
					</Item.Content>
					<Item.Actions>
						<Button
							variant="ghost"
							size="sm"
							disabled={unblocking[user.profileId]}
							onclick={() => onUnblock(user.profileId)}
						>
							{#if unblocking[user.profileId]}
								<SpinnerGap class="size-4 animate-spin" />
							{:else}
								Unblock
							{/if}
						</Button>
					</Item.Actions>
				</Item.Root>
			{/each}
		{/if}
	</main>
</div>